...
log = logging.getLogger('dmt.absoluted_intf')
...
class AbsolutedSSH(object):
    ...
    def find_absoluted_config(self):
        """
        Try to read configuration of remote absolute daemon. In particular we need parameters concering rpc
        configuration.
        :return: tuple (absoluted_running, absoluted_config_found, absoluted config file contents as dict)
                or error string in error occured
        """
        absoluted_running = False
        absoluted_config_found = False
        if not self.ssh:
            raise Exception('SSH session not ready')
        try:
            # find absoluted process id if running
            try:
                pids = self.remote_command('ps -C "absoluted" -o pid')
            except UnknownError:
                raise Exception('is absoluted running on the remote machine?')
            pid = None
            ...
            config = {}
            if pid:
                absoluted_running = True
                # using absoluted pid find its executable path and then .absolutecore directory and finally absolute.conf file
                ...
                if executables and len(executables) >= 1:
                    ...
                    if len(elems) == 2:
                        ...
                        absoluted_dir = os.path.dirname(executable)
                        absolute_conf_file = absoluted_dir + '/.absolutecore/absolute.conf'
                        conf_lines = []
                        try:
                            conf_lines = self.remote_command('cat ' + absolute_conf_file)
                        except Exception as e:
                            # probably error no such file or directory
                            # try to read absoluted's cwd + cmdline
                            cwd_lines = self.remote_command('ls -l /proc/' + str(pid) + '/cwd')
                            if cwd_lines:
                                elems = cwd_lines[0].split('->')
                                if len(elems) >= 2:
                                    cwd = elems[1]
                                    absolute_conf_file = cwd + '/.absolutecore/absolute.conf'
                                    try:
                                        conf_lines = self.remote_command('cat ' + absolute_conf_file)
                                    except Exception as e:
                                        # second method did not suceed, so assume, that conf file is located
                                        # i /home/<username>/.absolutecore directory
                                        absolute_conf_file = '/home/' + self.username + '/.absolutecore/absolute.conf'
                                        conf_lines = self.remote_command('cat ' + absolute_conf_file)
                        ...
                        absoluted_config_found = True
            return absoluted_running, absoluted_config_found, config
        except Exception as e:
            return str(e)
...
class AbsolutedIndexException(JSONRPCException):
    """
    Exception for notifying, that absolute daemon should have indexing option tuned on
    """
    def __init__(self, parent_exception):
        JSONRPCException.__init__(self, parent_exception.error)
        self.message = self.message + \
                       '\n\nMake sure the absolute daemon you are connecting to has the following options enabled in ' \
                       'its absolute.conf:\n\n' + \
                       'addressindex=1\n' + \
                       'spentindex=1\n' + \
                       'timestampindex=1\n' + \
                       'txindex=1\n\n' + \
                       'Changing these parameters requires to execute absoluted with "-reindex" option (linux: ./absoluted -reindex)'
                    ...
                    try:
                        try:
                            ...
                            if last_conn_reset_time:
                                raise AbsolutedConnectionError(e)  # switch to another config if possible
                            else:
                                last_exception = e
                            ...
                            # exceptions raised most likely by not functioning absoluted node; try to switch to another node
                            # if there is any in the config
                            log.warning('Error while calling of "' + str(func) + ' (3)". Details: ' + str(e))
                            raise AbsolutedConnectionError(e)
...
class AbsolutedInterface(WndUtils):
    def __init__(self, window,
        ...
        self.cur_conn_def: Optional['AbsoluteNetworkConnectionCfg'] = None
    ...
    def switch_to_next_config(self):
        """
        If there is another absoluted config not used recently, switch to it. Called only when there was a problem
    ...
    def open(self):
        """
        Opens connection to absolute RPC. If it fails, then the next enabled conn config will be used, if any exists.
        ...
        try:
            if not self.cur_conn_def:
                raise Exception('There is no connections to Absolute network enabled in the configuration.')
            ...
            while True:
                try:
                    ...
                    return False
                        ...
                        NoValidConnectionsError) as e:
                    # exceptions raised by not likely functioning absoluted node; try to switch to another node
                    # if there is any in the config
                    if not self.switch_to_next_config():
    ...
    def open_internal(self):
        """
        Try to establish connection to absolute RPC daemon for current connection config.
        ...
        if not self.active:
            ...
            if self.cur_conn_def.use_ssh_tunnel:
                # RPC over SSH
                if self.ssh is None:
                    self.ssh = AbsolutedSSH(self.cur_conn_def.ssh_conn_cfg.host, self.cur_conn_def.ssh_conn_cfg.port,
        ...
        if self.open():
            ...
            if verify_node:
                ...
                if self.app_config.is_testnet() and not node_under_testnet:
                    raise Exception('This RPC node works under Absolute MAINNET, but your current configuration is '
                                    'for TESTNET.')
                elif self.app_config.is_mainnet() and node_under_testnet:
                    raise Exception('This RPC node works under Absolute TESTNET, but your current configuration is '
                                    'for MAINNET.')
            return info
        else:
            raise Exception('Not connected')
    ...
    def read_protx_list(self):
        last_read_time = app_cache.get_value(f'ProtxLastReadTime_{self.app_config.absolute_network}', 0, int)
    ...
    def get_masternodelist(self, *args, data_max_age=MASTERNODES_CACHE_VALID_SECONDS,
        ...
        """
        Returns masternode list, read from the Absolute network or from the internal cache.
        ...
        if self.open():
            ...
            if len(args) == 1 and args[0] == 'json':
                last_read_time = app_cache.get_value(f'MasternodesLastReadTime_{self.app_config.absolute_network}', 0, int)
                ...
                if self.masternodes and data_max_age > 0 and \
                   int(time.time()) - last_read_time < data_max_age:
                    ...
                    try:
                        ...
                        app_cache.set_value(f'MasternodesLastReadTime_{self.app_config.absolute_network}', int(time.time()))
