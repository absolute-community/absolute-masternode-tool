...
from absolute_utils import bip32_path_n_to_string
from absoluted_intf import AbsolutedInterface, AbsolutedIndexException
...
class MainWindow(QMainWindow, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def __init__(self, app_dir):
        ...
        self.absoluted_intf = AbsolutedInterface(window=None,
        ...
        self.hw_session = HwSessionInfo(
            ...
            self.app_config,
            absoluted_intf=self.absoluted_intf)

        self.absoluted_info = {}
        self.is_absoluted_syncing = False
        self.absoluted_connection_ok = False
        self.connecting_to_absoluted = False
    ...
    def setupUi(self):
        ...
        self.absoluted_intf.window = self
        ...
        self.wdg_masternode = WdgMasternodeDetails(self, self.app_config, self.absoluted_intf)
        ...
        self.absoluted_intf.initialize(self.app_config)
    ...
    def closeEvent(self, event):
        ...
        self.finishing = True
        if self.absoluted_intf:
            self.absoluted_intf.disconnect()
    ...
    def load_configuration_from_file(self, file_name: str, ask_save_changes = True,
        ...
        try:
            self.disconnect_hardware_wallet()
            absolute_network_sav = self.app_config.absolute_network
            ...
            self.editing_enabled = False
            self.configuration_to_ui()
            self.absoluted_intf.reload_configuration()
            ...
            if file_name:
                ...
                self.update_config_files_mru_menu_items()
                if absolute_network_sav != self.app_config.absolute_network:
                    self.disconnect_hardware_wallet()
    ...
    def display_window_title(self):
        ...
        if self.app_config.absolute_network == 'TESTNET':
    ...
    def get_project_config_params_thread(self, ctrl, force_check):
        ...
        try:
            response = urllib.request.urlopen(
                'https://raw.githubusercontent.com/Bertrand256/absolute-masternode-tool/master/app-params.json',
    ...
    def on_action_open_settings_window_triggered(self):
        ...
        if res and dlg.get_is_modified():
            ...
            self.absoluted_intf.reload_configuration()
            if absolute_network_sav != self.app_config.absolute_network or hw_type_sav != self.app_config.hw_type:
                self.disconnect_hardware_wallet()
                self.app_config.reset_network_dependent_dyn_params()
            self.display_window_title()
            self.update_edit_controls_state()
    ...
    def show_connection_initiated(self):
        """Shows status information related to a initiated process of connection to a absolute RPC. """
        self.set_status_text1('<b>RPC network status:</b> trying %s...' % self.absoluted_intf.get_active_conn_description(), 'black')
    ...
    def show_connection_failed(self):
        ...
        self.set_status_text1('<b>RPC network status:</b> failed connection to %s' % self.absoluted_intf.get_active_conn_description(), 'red')

    def show_connection_successful(self):
        """Shows status information after successful connetion to a Absolute RPC node."""
        self.set_status_text1('<b>RPC network status:</b> OK (%s)' % self.absoluted_intf.get_active_conn_description(), 'green')

    def show_connection_disconnected(self):
        """Shows status message related to disconnection from Absolute RPC node."""
        self.set_status_text1('<b>RPC network status:</b> not connected', 'black')

    def connect_absolute_network(self, wait_for_check_finish=False, call_on_check_finished=None):
        """
        Connects do absolute daemon if not connected before and returnes if it was successful.
        ...
        def wait_for_synch_finished_thread(ctrl):
            """
            Thread waiting for absolute daemon to finish synchronizing.
            """
            mtx = QMutex()
            cond = QWaitCondition()
            try:
                ...
                while not ctrl.finish:
                    synced = self.absoluted_intf.issynchronized()
                    if synced:
                        self.is_absoluted_syncing = False
                        self.show_connection_successful()
                        break
                    mnsync = self.absoluted_intf.mnsync()
                    self.add_app_message(
                        DispMessage.ABSOLUTE_NET_CONNECTION,
                        'Absoluted is synchronizing: AssetID: %s, AssetName: %s' %
                        (str(mnsync.get('AssetID', '')), str(mnsync.get('AssetName', ''))), 'warn')
                    cond.wait(mtx, 5000)
                self.del_app_message(DispMessage.ABSOLUTE_NET_CONNECTION)
            except Exception as e:
                self.is_absoluted_syncing = False
                self.absoluted_connection_ok = False
                self.add_app_message(DispMessage.ABSOLUTE_NET_CONNECTION, str(e), 'error')
            finally:
                mtx.unlock()
                self.wait_for_absoluted_synced_thread = None

        def connect_thread(ctrl):
            """
            Test connection to absolute network inside a thread to avoid blocking GUI.
            :param ctrl: control structure to communicate with WorkerThread object (not used here)
            """
            try:
                synced = self.absoluted_intf.issynchronized()
                self.absoluted_info = self.absoluted_intf.getinfo(verify_node=True)
                self.absoluted_connection_ok = True
                if not synced:
                    logging.info("absoluted not synced")
                    if not self.is_absoluted_syncing and not (hasattr(self, 'wait_for_absoluted_synced_thread') and
                                                                  self.wait_for_absoluted_synced_thread is not None):
                        self.is_absoluted_syncing = True
                        self.wait_for_absoluted_synced_thread = self.run_thread(self, wait_for_synch_finished_thread, (),
                ...
                self.add_app_message(DispMessage.ABSOLUTE_NET_CONNECTION, err, 'error')
        ...
        def connect_finished():
            ...
            self.connecting_to_absoluted = False
            self.app_config.read_absolute_network_app_params(self.absoluted_intf)
        ...
        if self.app_config.is_config_complete():
            if not hasattr(self, 'check_conn_thread') or self.check_conn_thread is None:
                ...
                if hasattr(self, 'wait_for_absoluted_synced_thread') and self.wait_for_absoluted_synced_thread is not None:
                    if call_on_check_finished is not None:
                        # if a thread waiting for absoluted to finish synchronizing is running, call the callback function
                        call_on_check_finished()
                else:
                    self.connecting_to_absoluted = True
    ...
    def on_action_check_network_connection_triggered(self):
        self.test_absolute_network_connection()

    def test_absolute_network_connection(self):
        def connection_test_finished():
            ...
            if self.absoluted_connection_ok:
                self.show_connection_successful()
                if self.is_absoluted_syncing:
                    self.infoMsg('Connection successful, but Absolute daemon is synchronizing.')
                else:
                    self.infoMsg('Connection successful.')
            else:
                if self.absoluted_intf.last_error_message:
                    self.errorMsg('Connection error: ' + self.absoluted_intf.last_error_message)
                else:
        ...
        if self.app_config.is_config_complete():
            ...
            self.connect_absolute_network(call_on_check_finished=connection_test_finished)
        else:
        ...
        if not self.hw_client:
            try:
                try:
                    ...
                    if self.app_config.absolute_network == 'TESTNET':
                        # check if Absolute testnet is supported by this hardware wallet
                        found_testnet_support = False
                        if self.app_config.hw_type in (HWType.trezor, HWType.keepkey):
                            try:
                                path = absolute_utils.get_default_bip32_base_path(self.app_config.absolute_network)
                                path += "/0'/0/0"
                                path_n = absolute_utils.bip32_path_string_to_n(path)
                                addr = hw_intf.get_address(self.hw_session, path_n, False)
                                if addr and absolute_utils.validate_address(addr, self.app_config.absolute_network):
                                    found_testnet_support = True
                            except Exception as e:
                                if str(e).find('Invalid coin name') < 0:
                                    logging.exception('Failed when looking for Absolute testnet support')
                        elif self.app_config.hw_type == HWType.ledger_nano_s:
                            addr = hw_intf.get_address(self.hw_session,
                                                       absolute_utils.get_default_bip32_path(self.app_config.absolute_network))
                            if absolute_utils.validate_address(addr, self.app_config.absolute_network):
                                found_testnet_support = False

                        if not found_testnet_support:
                            url = get_note_url('DMT0002')
                            msg = f'Your hardware wallet device does not support Absolute TESTNET ' \
                ...
                try:
                    with open(file_name, 'r') as f_ptr:
                        bip44_wallet = Bip44Wallet(self.app_config.hw_coin_name, self.hw_session,
                                                   self.app_config.db_intf, self.absoluted_intf,
                            ...
                            if len(elems) >= 5 and not line.startswith('#'):
                                ...
                                def update_mn(in_mn):
                                    ...
                                    if len(ipelems) >= 2:
                                        ...
                                        in_mn.ip = mn_ipport
                                        in_mn.port = '18888'
                                    in_mn.collateralAddress = mn_absolute_addr
                        ...
                        if imported_cnt:
                            ...
                            if self.queryDlg(message=msg_text, buttons=QMessageBox.Yes | QMessageBox.No,
                                ...
                                # scan all Absolute addresses from imported masternodes for BIP32 path, starting from
                                # first standard Absolute BIP32 path
    ...
    def get_deterministic_tx(self, masternode: MasternodeConfig) -> Optional[Dict]:
        ...
        if masternode.dmn_tx_hash:
            try:
                protx = self.absoluted_intf.protx('info', masternode.dmn_tx_hash)
                if protx:
            ...
            if not protx:
                try:
                    ...
                    tx = self.absoluted_intf.getrawtransaction(masternode.dmn_tx_hash, 1, skip_cache=True)
        ...
        if not (protx_state and ((protx_state.get('service') == masternode.ip + ':' + masternode.port) or
            ...
            try:
                txes = self.absoluted_intf.protx('list', 'registered', True)
                for protx in txes:
                    protx_state = protx.get('state')
    ...
    def get_masternode_status_description_thread(self, ctrl, masternode: MasternodeConfig):
        ...
        if self.absoluted_connection_ok:
            ...
            self.absoluted_intf.get_masternodelist('json', data_max_age=30)  # read new data from the network
                                                                         # every 30 seconds
            if collateral_id:
                mn_info = self.absoluted_intf.masternodes_by_ident.get(collateral_id)
            else:
                mn_info = self.absoluted_intf.masternodes_by_ip_port.get(ip_port)

            block_height = self.absoluted_intf.getblockcount()
            ...
            if mn_info or dmn_tx:
                ...
                if not collateral_tx_mismatch:
                    if masternode.dmn_user_roles & DMN_ROLE_OWNER:
                        ...
                        tx_json = self.absoluted_intf.getrawtransaction(masternode.collateralTx, 1)
                        if tx_json:
                ...
                if dmn_tx:
                    ...
                    if dmn_tx_state:
                        ...
                        owner_address_cfg = masternode.get_dmn_owner_public_address(self.app_config.absolute_network)
                        if owner_address_network and owner_address_cfg and owner_address_network != owner_address_cfg:
                            ...
                            logging.warning(
                                ...
                                f'address from the app configuration: {owner_address_cfg}, address from the Absolute '
                                f'network: {owner_address_network}')

                        voting_address_network = dmn_tx_state.get('votingAddress')
                        voting_address_cfg = masternode.get_dmn_voting_public_address(self.app_config.absolute_network)
                        if voting_address_network and voting_address_cfg and voting_address_network != voting_address_cfg:
                            ...
                            logging.warning(
                                ...
                                f'address from the app configuration: {voting_address_cfg}, address from the Absolute '
                        ...
                        if not no_operator_pub_key:
                            ...
                            if operator_pubkey_network and operator_pubkey_cfg and \
                                ...
                                logging.warning(
                                    ...
                                    f'pubkey from the app configuration: {operator_pubkey_cfg}, pubkey from the Absolute '
                ...
                if masternode == self.cur_masternode:
                    ...
                    try:
                        if collateral_address:
                            collateral_bal = self.absoluted_intf.getaddressbalance([collateral_address])
                        ...
                        if collateral_address != payout_address and payout_address:
                            payout_bal = self.absoluted_intf.getaddressbalance([payout_address])
                        ...
                        if mn_info:
                            ...
                            if paid_block:
                                bh = self.absoluted_intf.getblockhash(paid_block)
                                blk = self.absoluted_intf.getblockheader(bh, 1)
                    ...
                    if dmn_tx and not dmn_tx.get('confirmations'):
                        ...
                        if self.absoluted_intf.is_protx_update_pending(self.cur_masternode.dmn_tx_hash):
                    ...
                    if owner_public_address_mismatch and not skip_data_mismatch:
                        errors.append('<td class="error" colspan="2">Owner Absolute address mismatch</td>')
                    ...
                    if voting_public_address_mismatch and not skip_data_mismatch:
                        errors.append('<td class="error" colspan="2">Voting Absolute address mismatch</td>')
                    if not dmn_tx:
            ...
            status = '<span style="color:red">Problem with connection to absoluted.</span>'
    ...
    def on_btnRefreshMnStatus_clicked(self):
        ...
        self.connect_absolute_network(wait_for_check_finish=True)
        if self.absoluted_connection_ok:
            try:
            ...
            enable_buttons()
            self.lblMnStatus.setText('')
            self.errorMsg('Absolute daemon not connected')
    ...
    def on_action_transfer_funds_for_cur_mn_triggered(self):
        ...
        if self.cur_masternode:
            ...
            if not self.cur_masternode.collateralBip32Path:
                ...
                self.errorMsg("Enter the masternode collateral Absolute address. You can use the 'left arrow' "
    ...
    def show_wallet_window(self, initial_mn: Optional[int]):
        ...
        """
        if not self.absoluted_intf.open():
            self.errorMsg('Absolute daemon not connected')
        else:
    ...
    def on_action_open_proposals_window_triggered(self):
        ui = ProposalsDlg(self, self.absoluted_intf)
        ui.exec_()
    ...
    def on_btnRegisterDmn_clicked(self, enabled):
        ...
        if self.cur_masternode:
            reg_dlg = reg_masternode_dlg.RegMasternodeDlg(self, self.app_config, self.absoluted_intf, self.cur_masternode,
    ...
    def update_registrar(self, show_upd_payout: bool, show_upd_operator: bool, show_upd_voting: bool):
        ...
        if self.cur_masternode:
            upd_dlg = upd_mn_registrar_dlg.UpdMnRegistrarDlg(
                self, self.app_config, self.absoluted_intf, self.cur_masternode,
