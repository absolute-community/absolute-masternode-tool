...
class MyTxApiInsight(TxApiInsight):
    ...
    def __init__(self, network, url, absoluted_inf, cache_dir, zcash=None):
    ...
    def fetch_json(self, url, resource, resourceid):
        if resource == 'tx':
            try:
                j = self.absoluted_inf.getrawtransaction(resourceid, 1, skip_cache=self.skip_cache)
                return j
            except Exception as e:
                raise
        else:
...
def sign_tx(hw_session: HwSessionInfo, utxos_to_spend: List[wallet_common.UtxoType],
    ...
    tx_api = MyTxApiInsight(insight_network, '', hw_session.absoluted_intf, hw_session.app_config.tx_cache_dir)
