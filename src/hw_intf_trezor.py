...
def is_absolute(coin):
    return coin["coin_name"].lower().startswith("absolute")
...
def json_to_tx(coin, data):
    ...
    if is_absolute(coin):
        ...
        if t.version == 3 and dip2_type != 0:
            ...
            t.extra_data = absolute_utils.num_to_varint(data["extraPayloadSize"]) + bytes.fromhex(
                data["extraPayload"]
            )
...
class MyTxApiInsight(TxApi):
    ...
    def __init__(self, network, url, absoluted_inf, cache_dir):
        TxApi.__init__(self, network)
        self.absoluted_inf = absoluted_inf
    ...
    def fetch_json(self, *path, **params):
        if path:
            if len(path) >= 2:
                if path[0] == 'tx':
                    try:
                        j = self.absoluted_inf.getrawtransaction(path[1], 1, skip_cache=self.skip_cache)
                        return j
                    except Exception as e:
                        raise
                else:
    ...
    def get_block_hash(self, block_number):
        return self.absoluted_inf.getblockhash(block_number)

    def current_height(self):
        return self.absoluted_inf.getheight()
...
def sign_tx(hw_session: HwSessionInfo, utxos_to_spend: List[wallet_common.UtxoType],
    ...
    insight_network = 'insight_absolute'
    if hw_session.app_config.is_testnet():
        insight_network += '_testnet'
    absolute_network = hw_session.app_config.absolute_network
    ...
    tx_api = MyTxApiInsight(coin, '', hw_session.absoluted_intf, hw_session.app_config.tx_cache_dir)
    ...
    for utxo_index, utxo in enumerate(utxos_to_spend):
        ...
        address_n = absolute_utils.bip32_path_string_to_n(utxo.bip32_path)
    ...
    for out in tx_outputs:
        ...
        if out.address[0] in absolute_utils.get_chain_params(absolute_network).B58_PREFIXES_SCRIPT_ADDRESS:
            stype = trezor_proto.OutputScriptType.PAYTOSCRIPTHASH
            logging.debug('Transaction type: PAYTOSCRIPTHASH' + str(stype))
        elif out.address[0] in absolute_utils.get_chain_params(absolute_network).B58_PREFIXES_PUBKEY_ADDRESS:
        ...
        if out.bip32_path:
            address_n = absolute_utils.bip32_path_string_to_n(out.bip32_path)
        else:
            address_n = None
...
def sign_message(hw_session: HwSessionInfo, bip32path, message):
    client = hw_session.hw_client
    address_n = absolute_utils.bip32_path_string_to_n(bip32path)
    try:
