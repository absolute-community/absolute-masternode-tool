...
class AppFeatueStatus(QObject):
    ...
    #  6: value read from the Absolute network (the highest priority by default)
    PRIORITY_DEFAULT = 0
...
class AppConfig(QObject):
    ...
    def __init__(self):
        ...
        # List of Absolute network configurations. Multiple conn configs advantage is to give the possibility to use
        # another config if particular one is not functioning (when using "public" RPC service, it could be node's
        # maintanance)
        self.absolute_net_configs = []

        # to distribute the load evenly over "public" RPC services, we choose radom connection (from enabled ones)
        # if it is set to False, connections will be used accoording to its order in absolute_net_configs list
        self.random_absolute_net_config = True

        # list of all enabled absoluted configurations (AbsoluteNetworkConnectionCfg) - they will be used accourding to
        # the order in list
        self.active_absolute_net_configs = []

        # list of misbehaving absolute network configurations - they will have the lowest priority during next
        # connections
        ...
        self._absolute_blockchain_info = {}
        ...
        self.absolute_network = 'MAINNET'

        self.block_explorer_tx_mainnet = 'https://explorer.absify.me/ext/gettx/%TXID%'
        self.block_explorer_addr_mainnet = 'https://explorer.absify.me/ext/getbalance/%ADDRESS%'
        self.block_explorer_tx_testnet = 'https://testnet.absify.me/tx/%TXID%'
        self.block_explorer_addr_testnet = 'https://testnet.absify.me/address/%ADDRESS%'
        self.tx_api_url_mainnet = 'https://explorer.absify.me/'
        self.tx_api_url_testnet = 'https://testnet.absify.me/'
        self.absolute_central_proposal_api = 'https://proposal.absify.me/'
        self.absolute_nexus_proposal_api = 'https://proposal.absify.me/proposals/%HASH%'

        # public RPC connection configurations
        self.public_conns_mainnet: Dict[str, AbsoluteNetworkConnectionCfg] = {}
        self.public_conns_testnet: Dict[str, AbsoluteNetworkConnectionCfg] = {}
        ...
        try:
            self.default_rpc_connections = self.decode_connections(default_config.absoluted_default_connections)
        except Exception:
    ...
    def save_cache_settings(self):
        if self.feature_register_dmn_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_REGISTER_DMN_AUTOMATIC_' + self.absolute_network,
                                self.feature_register_dmn_automatic.get_value())
        if self.feature_update_registrar_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_UPDATE_REGISTRAR_AUTOMATIC_' + self.absolute_network,
                                self.feature_update_registrar_automatic.get_value())
        if self.feature_update_service_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_UPDATE_SERVICE_AUTOMATIC_' + self.absolute_network,
                                self.feature_update_service_automatic.get_value())
        if self.feature_revoke_operator_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_REVOKE_OPERATOR_AUTOMATIC_' + self.absolute_network,
                                self.feature_revoke_operator_automatic.get_value())

    def restore_cache_settings(self):
        ena = app_cache.get_value('FEATURE_REGISTER_AUTOMATIC_DMN_' + self.absolute_network, True, bool)
        self.feature_register_dmn_automatic.set_value(ena, AppFeatueStatus.PRIORITY_APP_CACHE)
        ena = app_cache.get_value('FEATURE_UPDATE_REGISTRAR_AUTOMATIC_' + self.absolute_network, True, bool)
        self.feature_update_registrar_automatic.set_value(ena, AppFeatueStatus.PRIORITY_APP_CACHE)
        ena = app_cache.get_value('FEATURE_UPDATE_SERVICE_AUTOMATIC_' + self.absolute_network, True, bool)
        self.feature_update_service_automatic.set_value(ena, AppFeatueStatus.PRIORITY_APP_CACHE)
        ena = app_cache.get_value('FEATURE_REVOKE_OPERATOR_AUTOMATIC_' + self.absolute_network, True, bool)
        self.feature_revoke_operator_automatic.set_value(ena, AppFeatueStatus.PRIORITY_APP_CACHE)

    def copy_from(self, src_config):
        self.absolute_network = src_config.absolute_network
        self.absolute_net_configs = copy.deepcopy(src_config.absolute_net_configs)
        self.random_absolute_net_config = src_config.random_absolute_net_config
        ...
        self.absolute_central_proposal_api = src_config.absolute_central_proposal_api
    ...
    def clear_configuration(self):
        ...
        :return:
        """
        self.absolute_net_configs.clear()
        self.active_absolute_net_configs.clear()
    ...
    def read_from_file(self, hw_session: HwSessionInfo, file_name: Optional[str] = None,
        ...
        if os.path.exists(file_name):
            ...
            try:
                ...
                absolute_network = config.get(section, 'absolute_network', fallback='MAINNET')
                if absolute_network not in ('MAINNET', 'TESTNET'):
                    logging.warning(f'Invalid absolute_network value: {absolute_network}')
                    absolute_network = 'MAINNET'
                self.absolute_network = absolute_network
                ...
                self.random_absolute_net_config = self.value_to_bool(config.get(section, 'random_absolute_net_config',
                                                                            fallback='1'))
                ...
                for section in config.sections():
                    try:
                        if re.match('MN\d', section):
                            ...
                            cfg = AbsoluteNetworkConnectionCfg('rpc')
                            ...
                            if cfg.host.lower() == 'test.stats.absolute.org':
                                skip_adding = True
                            ...
                            if not skip_adding:
                                self.absolute_net_configs.append(cfg)
    ...
    def save_to_file(self, hw_session: HwSessionInfo, file_name: Optional[str] = None,
        ...
        config.set(section, 'absolute_network', self.absolute_network)
        if not self.hw_type:
            self.hw_type = HWType.trezor
        ...
        config.set(section, 'random_absolute_net_config', '1' if self.random_absolute_net_config else '0')
        ...
        # save absolute network connections
        for idx, cfg in enumerate(self.absolute_net_configs):
    ...
    def apply_remote_app_params(self):
        def get_feature_config_remote(symbol) -> Tuple[Optional[bool], Optional[int], Optional[str]]:
            ...
            if features:
                ...
                if feature:
                    a = feature.get(self.absolute_network.lower())
                    if a:
    ...
    def read_absolute_network_app_params(self, absoluted_intf):
        """ Read parameters having impact on the app's behavior (sporks/dips) from the Absolute network. Called
    ...
    def get_default_protocol(self) -> int:
        prot = None
        if self._remote_app_params:
            dp = self._remote_app_params.get('defaultAbsolutedProtocol')
            if dp:
                prot = dp.get(self.absolute_network.lower())
        return prot
    ...
    def is_config_complete(self):
        for cfg in self.absolute_net_configs:
            if cfg.enabled:
                return True
        return False
    ...
    def prepare_conn_list(self):
        """
        Prepare list of enabled connections for connecting to absolute network. 
        :return: list of AbsoluteNetworkConnectionCfg objects order randomly (random_absolute_net_config == True) or according 
        ...
        for cfg in self.absolute_net_configs:
            if cfg.enabled and self.is_testnet() == cfg.testnet:
                tmp_list.append(cfg)
        if self.random_absolute_net_config:
            ordered_list = []
            while len(tmp_list):
                idx = randint(0, len(tmp_list)-1)
                ordered_list.append(tmp_list[idx])
                del tmp_list[idx]
            self.active_absolute_net_configs = ordered_list
        else:
            self.active_absolute_net_configs = tmp_list

    def get_ordered_conn_list(self):
        if not self.active_absolute_net_configs:
            self.prepare_conn_list()
        return self.active_absolute_net_configs
    ...
    def decode_connections(self, raw_conn_list) -> List['AbsoluteNetworkConnectionCfg']:
        """
        Decodes list of dicts describing connection to a list of AbsoluteNetworkConnectionCfg objects.
        :param raw_conn_list: 
    ...
    def decode_connections_json(self, conns_json):
        ...
        :return: list of AbsoluteNetworkConnectionCfg objects or None if there was an error while importing
        """
        try:
    ...
    def import_connections(self, in_conns, force_import, limit_to_network: Optional[str]):
        ...
        :param in_conns: list of AbsoluteNetworkConnectionCfg objects.
        ...
        if in_conns:
            ...
            for conn in self.absolute_net_configs:
                if conn.testnet:
            ...
            for nc in in_conns:
                if (self.absolute_network == 'MAINNET' and nc.testnet == False) or \
                   (self.absolute_network == 'TESTNET' and nc.testnet == True) or not limit_to_network:
                    id = nc.get_conn_id()
                    ...
                    if not conn:
                        if force_import or not app_cache.get_value('imported_default_conn_' + nc.get_conn_id(),
                           ...
                           (testnet_conn_count == 0 and nc.testnet) or  (mainnet_conn_count == 0 and nc.mainnet):
                            ...
                            self.absolute_net_configs.append(nc)
    ...
    def get_conn_cfg_by_id(self, id):
        """
        Returns AbsoluteNetworkConnectionCfg object by its identifier or None if does not exists.
        :param id: Identifier of the sought connection.
        :return: AbsoluteNetworkConnectionCfg object or None if does not exists.
        """
        for conn in self.absolute_net_configs:
    ...
    def is_testnet(self) -> bool:
        return self.absolute_network == 'TESTNET'

    def is_mainnet(self) -> bool:
        return self.absolute_network == 'MAINNET'

    @property
    def hw_coin_name(self):
        if self.is_testnet():
            return 'Absolute Testnet'
        else:
            return 'Absolute'

    def get_block_explorer_tx(self):
        if self.absolute_network == 'MAINNET':
...
class MasternodeConfig:
    ...
    def get_current_key_for_voting(self, app_config: AppConfig, absoluted_intf):
        return self.dmn_voting_private_key

    def get_dmn_owner_public_address(self, absolute_network) -> Optional[str]:
        if self.__dmn_owner_key_type == InputKeyType.PRIVATE:
            if self.__dmn_owner_private_key:
                address = absolute_utils.wif_privkey_to_address(self.__dmn_owner_private_key, absolute_network)
                return address
        else:
    ...
    def get_dmn_owner_pubkey_hash(self) -> Optional[str]:
        if self.dmn_owner_key_type == InputKeyType.PRIVATE:
            if self.__dmn_owner_private_key:
                pubkey = absolute_utils.wif_privkey_to_pubkey(self.__dmn_owner_private_key)
                pubkey_bin = bytes.fromhex(pubkey)
                pub_hash = bitcoin.bin_hash160(pubkey_bin)
                return pub_hash.hex()
        else:
            if self.__dmn_owner_address:
                ret = absolute_utils.address_to_pubkey_hash(self.__dmn_owner_address)
                if ret:
                    return ret.hex()
        return ''

    def get_dmn_voting_public_address(self, absolute_network) -> Optional[str]:
        if self.__dmn_voting_key_type == InputKeyType.PRIVATE:
            if self.__dmn_voting_private_key:
                address = absolute_utils.wif_privkey_to_address(self.__dmn_voting_private_key, absolute_network)
                return address
        else:
    ...
    def get_dmn_voting_pubkey_hash(self) -> Optional[str]:
        if self.__dmn_voting_key_type == InputKeyType.PRIVATE:
            if self.__dmn_voting_private_key:
                pubkey = absolute_utils.wif_privkey_to_pubkey(self.__dmn_voting_private_key)
                pubkey_bin = bytes.fromhex(pubkey)
                pub_hash = bitcoin.bin_hash160(pubkey_bin)
                return pub_hash.hex()
        else:
            if self.__dmn_voting_address:
                ret = absolute_utils.address_to_pubkey_hash(self.__dmn_voting_address)
                if ret:
                    return ret.hex()
        return ''
    ...
    def get_dmn_operator_pubkey(self) -> Optional[str]:
        if self.__dmn_operator_key_type == InputKeyType.PRIVATE:
            if self.__dmn_operator_private_key:
                pubkey = absolute_utils.bls_privkey_to_pubkey(self.__dmn_operator_private_key)
                return pubkey
        else:
            return self.__dmn_operator_public_key
        return ''
...
class AbsoluteNetworkConnectionCfg(object):
    ...
    def identical(self, cfg2):
        ...
        :param cfg2: AbsoluteNetworkConnectionCfg object to compare
    ...
    def __deepcopy__(self, memodict):
        newself = AbsoluteNetworkConnectionCfg(self.method)
    ...
    def is_http_proxy(self):
        ...
        proxy uses SSL while normal, "local" absoluted does not. 
        """
        if self.__use_ssl:
            return True
        else:
