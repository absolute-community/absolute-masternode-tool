...
from absoluted_intf import AbsolutedIndexException
...
class ListCollateralTxsDlg(QDialog, ui_find_coll_tx_dlg.Ui_ListCollateralTxsDlg, wnd_utils.WndUtils):
    ...
    def display_title(self):
        if len(self.utxos):
            if self.read_only:
                msg = f'<span>Found 2500 Absolute transaction(s):</span>'
            else:
            ...
            self.lblMessage.setText('<span style="color:red"><b>Found no unspent 2500 Absolute transactions in your '
                                    'wallet</b></span>')
            self.lblMessage.setVisible(True)
