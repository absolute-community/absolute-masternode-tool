...
import app_config
import app_cache
from app_config import AppConfig, AbsoluteNetworkConnectionCfg
from absoluted_intf import AbsolutedInterface, control_rpc_call
...
class ConfigDlg(QDialog, Ui_ConfigDlg, WndUtils):
    def __init__(self, parent, app_config: AppConfig):
        ...
        # list of connections from self.local_config.absolute_net_configs split on separate lists for mainnet and testnet
        self.connections_mainnet = []
        self.connections_testnet = []
        self.connections_current = None
        self.current_network_cfg : Optional[AbsoluteNetworkConnectionCfg] = None
    ...
    def setupUi(self):
        ...
        if self.local_config.is_mainnet():
            self.cboAbsoluteNetwork.setCurrentIndex(0)
            self.connections_current = self.connections_mainnet
        else:
            self.cboAbsoluteNetwork.setCurrentIndex(1)
            self.connections_current = self.connections_testnet
        for cfg in self.local_config.absolute_net_configs:
            if cfg.testnet:
        ...
        if len(self.local_config.absolute_net_configs):
    ...
    @pyqtSlot(int)
    def on_cboAbsoluteNetwork_currentIndexChanged(self, index):
        """Executed after changing configuration between MAINNET and TESTNET."""
        if not self.disable_cfg_update:
            if index == 0:
                self.connections_current = self.connections_mainnet
                self.local_config.absolute_network = 'MAINNET'
            else:
                self.connections_current = self.connections_testnet
                self.local_config.absolute_network = 'TESTNET'
            self.display_connection_list()
            self.set_modified()
            self.lstConns.setCurrentRow(0)
        ...
        try:
            ...
            if isinstance(conns, list) and len(conns):
                ...
                if self.queryDlg('Do you really want to import connection(s) from clipboard?',
                    ...
                    self.local_config.absolute_net_configs.clear()
                    self.local_config.absolute_net_configs.extend(self.connections_mainnet)
                    self.local_config.absolute_net_configs.extend(self.connections_testnet)

                    added, updated = self.local_config.import_connections(
                        conns, force_import=True, limit_to_network=self.local_config.absolute_network)
                    for cfg in added:
                        cfg.enabled = True
    ...
    def on_btnRestoreDefault_clicked(self, enabled):
        if self.queryDlg('Do you really want to restore default connection(s)?',
                         buttons=QMessageBox.Yes | QMessageBox.Cancel,
                         default_button=QMessageBox.Yes, icon=QMessageBox.Information) == QMessageBox.Yes:
            cfgs = self.local_config.decode_connections(default_config.absoluted_default_connections)
            if cfgs:
                # update the main list containing connections configuration from separate  lists dedicated
                # to mainnet and testnet - it'll be used by the import_connection method
                self.local_config.absolute_net_configs.clear()
                ...
                added, updated = self.local_config.import_connections(
                    cfgs, force_import=True, limit_to_network=self.local_config.absolute_network)
                self.connections_current.extend(added)
                if added or updated:
                    row_selected = self.lstConns.currentRow()
                    self.display_connection_list()
    ...
    def on_action_new_connection_triggered(self):
        ...
        cfg.testnet = True if self.cboAbsoluteNetwork.currentIndex() == 1 else False
    ...
    def on_chbRandomConn_toggled(self, checked):
        if not self.disable_cfg_update:
            self.local_config.random_absolute_net_config = checked
            self.set_modified()
    ...
    def update_connection_details_ui(self):
        ...
        try:
            ...
            self.chbRandomConn.setChecked(self.local_config.random_absolute_net_config)
        finally:
    ...
    def on_btnSshReadRpcConfig_clicked(self):
        """Read the configuration of a remote RPC node from the node's absolute.conf file."""
        if self.current_network_cfg:
            ...
            if not ok or not username:
                return
            from absoluted_intf import AbsolutedSSH
            ssh = AbsolutedSSH(host, int(port), username, auth_method=auth_method, private_key_path=private_key_path)
            try:
                if ssh.connect():
                    absoluted_conf = ssh.find_absoluted_config()
                    self.disable_cfg_update = True
                    if isinstance(absoluted_conf, tuple) and len(absoluted_conf) >= 3:
                        if not absoluted_conf[0]:
                            self.infoMsg('Remore Absolute daemon seems to be shut down')
                        elif not absoluted_conf[1]:
                            self.infoMsg('Could not find remote absoluted.conf file')
                        else:
                            file = absoluted_conf[2]
                            rpcuser = file.get('rpcuser', '')
                            rpcpassword = file.get('rpcpassword', '')
                            rpcport = file.get('rpcport', '18889')
                            modified = False
                            if rpcuser:
                            ...
                            if file.get('server', '1') == '0':
                                self.warnMsg("Remote absolute.conf parameter 'server' is set to '0', so RPC interface will "
                                             "not work.")
                            if not rpcuser:
                                self.warnMsg("Remote absolute.conf parameter 'rpcuser' is not set, so RPC interface will  "
                                             "not work.")
                            if not rpcpassword:
                                self.warnMsg("Remote absolute.conf parameter 'rpcpassword' is not set, so RPC interface will  "
                                             "not work.")
                        self.update_connection_details_ui()
                    elif isinstance(absoluted_conf, str):
                        self.warnMsg("Couldn't read remote absoluted configuration file due the following error: " +
                                     absoluted_conf)
                    ssh.disconnect()
            except Exception as e:
                self.errorMsg(str(e))
    ...
    def on_btnTestConnection_clicked(self):
        if self.current_network_cfg:
            ...
            absoluted_intf = AbsolutedInterface(window=self)
            absoluted_intf.initialize(self.local_config, connection=self.current_network_cfg,
                                  for_testing_connections_only=True)
            try:
                info = absoluted_intf.getinfo(verify_node=True)
                if info:
                    try:
                        ret = absoluted_intf.rpc_call(True, False, "checkfeaturesupport", "enhanced_proxy")
                    except Exception as e:
                        ret = None
