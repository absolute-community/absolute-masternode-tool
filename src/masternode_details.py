...
class WdgMasternodeDetails(QWidget, ui_masternode_details.Ui_WdgMasternodeDetails):
    ...
    def __init__(self, main_dlg, app_config, absoluted_intf):
    ...
    def setupUi(self):
        ...
        self.act_view_as_owner_public_address = QAction('View as Absolute address', self)
        ...
        self.act_view_as_voting_public_address = QAction('View as Absolute address', self)
                    ...
                    if self.masternode.dmn_owner_key_type == InputKeyType.PRIVATE:
                        if self.act_view_as_owner_private_key.isChecked():
                            ...
                            if self.masternode.dmn_owner_private_key:
                                ret = absolute_utils.wif_privkey_to_address(self.masternode.dmn_owner_private_key,
                                                                        self.app_config.absolute_network)
                        elif self.act_view_as_owner_public_key.isChecked():
                            if self.masternode.dmn_owner_private_key:
                                ret = absolute_utils.wif_privkey_to_pubkey(self.masternode.dmn_owner_private_key)
                        elif self.act_view_as_owner_public_key_hash.isChecked():
                            if self.masternode.dmn_owner_private_key:
                                pubkey = absolute_utils.wif_privkey_to_pubkey(self.masternode.dmn_owner_private_key)
                    ...
                    if self.masternode.dmn_voting_key_type == InputKeyType.PRIVATE:
                        if self.act_view_as_voting_private_key.isChecked():
                            ...
                            if self.masternode.dmn_voting_private_key:
                                ret = absolute_utils.wif_privkey_to_address(self.masternode.dmn_voting_private_key,
                                                                        self.app_config.absolute_network)
                        elif self.act_view_as_voting_public_key.isChecked():
                            if self.masternode.dmn_voting_private_key:
                                ret = absolute_utils.wif_privkey_to_pubkey(self.masternode.dmn_voting_private_key)
                        elif self.act_view_as_voting_public_key_hash.isChecked():
                            if self.masternode.dmn_voting_private_key:
                                pubkey = absolute_utils.wif_privkey_to_pubkey(self.masternode.dmn_voting_private_key)
                                pubkey_bin = bytes.fromhex(pubkey)
                                pub_hash = bitcoin.bin_hash160(pubkey_bin)
                                ret = pub_hash.hex()
                        else:
    ...
    def generate_priv_key(self, pk_type:str, edit_control: QLineEdit, compressed: bool):
        ...
        if pk_type == 'operator':
            pk = absolute_utils.generate_bls_privkey()
        else:
            pk = absolute_utils.generate_wif_privkey(self.app_config.absolute_network, compressed=compressed)
    ...
    def on_btnLocateCollateral_clicked(self, checked):
        ...
        if utxos:
            ...
            if utxos is not None:
                WndUtils.warnMsg('Couldn\'t find any 1000 Absolute UTXO in your wallet.')
    ...
    def get_collateral_tx_address_thread(self, ctrl: CtrlObject,
        ...
        break_scanning = False
        txes_cnt = 0
        msg = 'Scanning wallet transactions for 2500 Absolute UTXOs.<br>' \
