...
from absoluted_intf import AbsolutedIndexException, Masternode
...
# will display data from cache, instead of requesting them from a absolute daemon, which is
# more time consuming.
...
class VotingMasternode(AttrsProtected):
    def __init__(self, masternode, masternode_config):
        ...
        :param masternode: ref to an object storing mn information read from the network (absoluted_intf.Masternode)
...
class Proposal(AttrsProtected):
    ...
    def apply_values(self, masternodes, last_superblock_time, next_superblock_datetime):
        ...
        if not self.get_value('title'):
            # if title value is not set (it's an external attribute, from absolutecentral) then copy value from the
            # name column
...
class ProposalsDlg(QDialog, ui_proposals.Ui_ProposalsDlg, wnd_utils.WndUtils):
    def __init__(self, parent, absoluted_intf):
        ...
        for idx, mn in enumerate(self.app_config.masternodes):
            ...
            if mn_ident not in mn_idents:
                if mn.dmn_voting_key_type == InputKeyType.PRIVATE:
                    voting_key = mn.get_current_key_for_voting(self.app_config, self.absoluted_intf)
                    if voting_key:
                        if absolute_utils.validate_wif_privkey(voting_key, self.app_config.absolute_network):
    ...
    def display_budget_summary(self):
        ...
        if self.next_budget_approved is not None:
            budget_approved = f'<td><b>Budget approved:</b> {app_utils.to_string(round(self.next_budget_approved))} Absolute '
        ...
        if self.next_budget_requested is not None:
            budget_requested = f'<td><b>Budget requested:</b> {app_utils.to_string(round(self.next_budget_requested))} Absolute '
        ...
        if self.votes_loaded:
            if self.next_budget_approved_by_user_yes_votes is not None:
                budget_approved_user_yes = \
                    ...
                    f'{app_utils.to_string(round(self.next_budget_approved_by_user_yes_votes))} Absolute '
                if self.next_budget_amount:
                    budget_approved_user_yes += \
        ...
        message = '<html><head></head><style>td{padding-right:10px;white-space:nowrap;}</style><body>' \
                  ...
                  f'{bra}<tr><td><b>Budget available:</b> {app_utils.to_string(round(self.next_budget_amount))} Absolute</td>' \
                  f'<td><b>Budget left:</b> {app_utils.to_string(round(self.next_budget_left))} Absolute</td></tr>' \
    ...
    def read_proposals_from_network(self):
        """ Reads proposals from the Absolute network. """
        ...
        try:
            ...
            log.info('Reading proposals from the Absolute network.')
            begin_time = time.time()
            proposals_new = self.absoluted_intf.gobject("list", "valid", "proposals")
            ...
            if len(proposals_new) > 0:
                if errors < len(proposals_new)/10:
                    try:
                        ...
                        for prop in self.proposals:
                            ...
                            if prop.marker:
                                if not prop.db_id:
                                    ...
                                    # absoluted sometimes does not return some proposals, so they are deactivated id the db
                        ...
                        # delete proposals which no longer exists in tha Absolute network
                        rows_removed = False
                ...
                log.warning('No proposals returned from absoluted.')
            ...
            log.exception('Exception wile reading proposals from Absolute network.')
            self.display_message('')
            self.errorMsg('Error while reading proposals data from the Absolute network: ' + str(e))
            raise
    ...
    def read_governance_data(self):
        try:
            ...
            self.governanceinfo = self.absoluted_intf.getgovernanceinfo()
            ...
            self.next_budget_amount = float(self.absoluted_intf.getsuperblockbudget(self.next_superblock))

            # superblocks occur every 16616 blocks (approximately 28.8 days)
            self.cur_block_height = self.absoluted_intf.getblockcount()
            ...
            self.errorMsg("Couldn't read governance info from the Absolute network. "
    ...
    def get_block_timestamp(self, superblock: int):
        ...
        if ts is None:
            bhash = self.absoluted_intf.getblockhash(superblock)
            bh = self.absoluted_intf.getblockheader(bhash)
            ts = bh['time']
    ...
    def read_data_thread(self, ctrl):
        ...
        try:
            self.display_message('Connecting to Absolute daemon, please wait...')
            if not self.absoluted_intf.open():
                ...
                try:
                    ...
                    mns = self.absoluted_intf.get_masternodelist('json')
                ...
                except AbsolutedIndexException as e:
                    self.errorMsg(str(e))
            ...
            if not self.finishing:
                # read additional data from external sources, if configured (AbsoluteCentral)
                proposals = []
    ...
    def read_external_attibutes(self, proposals):
        """Method reads additional proposal attributes from an external source such as AbsoluteCentral.org/AbsoluteNexus.org
        ...
        try:
            url = self.app_config.absolute_central_proposal_api
            if url:
    ...
    def read_voting_from_network(self, force_reload_all, proposals: List[Proposal]):
        """
        Retrieve from a Absolute daemon voting results for all defined masternodes, for all visible Proposals.
                    ...
                    try:
                        ...
                        for row_idx, prop in enumerate(proposals):
                            try:
                                ...
                                tm_begin = time.time()
                                try:
                                    votes = self.absoluted_intf.rpc_call(False, False, 'gobject', 'getcurrentvotes',
                                                                     prop.get_value('hash'))
                                except Exception:
    ...
    def refresh_proposals_thread(self, ctrl):
        ...
        proposals = []
        if self.app_config.read_proposals_external_attributes:
            # select proposals for which we read additional data from external sources as AbsoluteCentral.org
            for prop in self.proposals:
                if not prop.ext_attributes_loaded:
                    proposals.append(prop)
        if proposals and not self.finishing:
    ...
    def refresh_details_tabs(self):
        try:
            ...
            if self.current_proposal:
                ...
                if not re.match('.*absolutecentral\.org', url.lower()):
                    dc_url = 'https://www.absolutecentral.org/p/' + prop.get_value('name')
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding" style="white-space:nowrap"><span>{app_utils.to_string(prop.get_value('payment_amount'))} Absolute&#47;cycle ({cycles_str}, {app_utils.to_string(prop.get_value('payment_amount_total'))} Absolute total)
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding">{app_utils.to_string(total_amount_requested)} Absolute{total_pct_requested}</td>
        </tr>
        <tr class="main-row">
            <td class="first-col-label">Approved from the next budget:</td>
            <td class="padding">{app_utils.to_string(total_amount_approved)} Absolute{total_pct_approved}</td>
        </tr>
    </tbody>
</table>
 </body>
        ...
        for prop in proposal_list:
            ...
            for vote_idx, mn_info in enumerate(masternodes):
                ...
                try:
                    ...
                    step = 2
                    vote_sig = absolute_utils.ecdsa_sign(
                        serialize_for_sig,
                        mn_info.masternode_config.get_current_key_for_voting(self.app_config, self.absoluted_intf),
                        self.app_config.absolute_network)

                    step = 3
                    v_res = self.absoluted_intf.voteraw(
                    ...
                    if step in (1, 4):
                        ...
                        log.info('masternode_pub_key: %s' %
                                     str(absolute_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.absoluted_intf))))
                        log.info('masternode_pub_key_hash: %s' %
                                     str(absolute_utils.pubkey_to_address(absolute_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.absoluted_intf)), self.app_config.absolute_network)))
