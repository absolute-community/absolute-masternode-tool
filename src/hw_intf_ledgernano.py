...
class btchip_dmt(btchip):
    ...
    def getTrustedInput(self, transaction, index):
        ...
        if transaction.extra_data:
            # Absolute DIP2 extra data: By appending data to the 'lockTime' transfer we force the device into the
    ...
    def process_ledger_exceptions_int(*args, **kwargs):
        try:
            ...
            if (e.sw in (0x6d00, 0x6700)):
                e.message += '\n\nMake sure the Absolute app is running on your Ledger device.'
            elif (e.sw == 0x6982):
...
def sign_tx(hw_session: HwSessionInfo, utxos_to_spend: List[wallet_common.UtxoType],
    ...
    for utxo in utxos_to_spend:
        if utxo.txid not in rawtransactions:
            tx = hw_session.absoluted_intf.getrawtransaction(utxo.txid, 1, skip_cache=False)
            if tx and tx.get('hex'):
                tx_raw = tx.get('hex')
            else:
                tx_raw = hw_session.absoluted_intf.getrawtransaction(utxo.txid, 0, skip_cache=False)
    ...
    for idx, utxo in enumerate(utxos_to_spend):
        ...
        if data['version'] == 3 and dip2_type != 0:
            ...
            prev_transaction.extra_data = absolute_utils.num_to_varint(data["extraPayloadSize"]) + bytes.fromhex(
                data["extraPayload"])
        else:
    ...
    for out in tx_outputs:
        ...
        output.script = compose_tx_locking_script(out.address, hw_session.app_config.absolute_network)
