...
class WalletDlg(QDialog, ui_wallet_dlg.Ui_WalletDlg, WndUtils):
    ...
    def __init__(self, main_ui, initial_mn_sel: int):
        ...
        self.absoluted_intf: AbsolutedInterface = main_ui.absoluted_intf
        self.db_intf: DBCache = main_ui.app_config.db_intf
        self.bip44_wallet = Bip44Wallet(self.app_config.hw_coin_name, self.hw_session, self.db_intf, self.absoluted_intf,
    ...
    def restore_cache_settings(self):
        ...
        # base bip32 path:
        path = app_cache.get_value(CACHE_ITEM_HW_ACCOUNT_BASE_PATH.replace('%NETWORK%', self.app_config.absolute_network),
                                   absolute_utils.get_default_bip32_base_path(self.app_config.absolute_network), str)
        if absolute_utils.validate_bip32_path(path):
            self.hw_account_base_bip32_path = path
        else:
            self.hw_account_base_bip32_path = absolute_utils.get_default_bip32_base_path(self.app_config.absolute_network)
        ...
        if self.initial_mn_sel is None:
            sel_hashes = app_cache.get_value(
                CACHE_ITEM_UTXO_SRC_MASTRNODE.replace('%NETWORK%', self.app_config.absolute_network), [], list)
        ...
        enc_json_str = app_cache.get_value(CACHE_ITEM_LAST_RECIPIENTS.replace('%NETWORK%', self.app_config.absolute_network), None, str)
        if enc_json_str:
            try:
    ...
    def save_cache_settings(self):
        ...
        app_cache.set_value(CACHE_ITEM_HW_ACCOUNT_BASE_PATH.replace('%NETWORK%', self.app_config.absolute_network),
                            self.hw_account_base_bip32_path)
        ...
        for mna in self.selected_mns:
            if mna.address.address:
                ...
                sel_hashes.append(h)
        app_cache.set_value(CACHE_ITEM_UTXO_SRC_MASTRNODE.replace('%NETWORK%', self.app_config.absolute_network),
                            sel_hashes)
        ...
        app_cache.set_value(CACHE_ITEM_LAST_RECIPIENTS.replace('%NETWORK%', self.app_config.absolute_network), rcp_data)
    ...
    def on_btnSend_clicked(self):
        """
        Sends funds to Absolute address specified by user.
        """
        try:
            self.allow_fetch_transactions = False
            self.enable_synch_with_main_thread = False
            ...
            if len(tx_inputs):
                ...
                #  - the utxo Absolute (signing) address matches the hardware wallet address for a given path
                for utxo_idx, utxo in enumerate(tx_inputs):
                    ...
                    if utxo.is_collateral:
                        if self.queryDlg(
                                "Warning: you are going to transfer masternode's collateral (2500 Absolute) transaction "
                    ...
                    if addr_hw != utxo.address:
                        self.errorMsg("<html style=\"font-weight:normal\">Absolute address inconsistency between UTXO "
                ...
                try:
                    ...
                    if tx_outputs:
                        ...
                        if len(tx_hex) > 90000:
                            ...
                            tx_dlg = TransactionDlg(self, self.main_ui.app_config, self.absoluted_intf, tx_hex, use_is,
...
<tr><td class="lbl">Balance</td><td>{app_utils.to_string(balance/1e8)} Absolute</td></tr>
<tr><td class="lbl">Received</td><td>{app_utils.to_string(received/1e8)} Absolute</td></tr>
</table>
</body>
"""
