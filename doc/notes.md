...
in passphrase in Keepkey will result in different Absolute addresses than those that would 
...
Keep in mind that the choice of encoding type affects the Absolute addresses generated for the 
...
## Note DMT0002
Current versions of the all three hardware wallets supported by the application do not support Absolute TESTNET. It is possible though, making a few modifications to the firmware source code of Trezor and KeepKey devices, to make them support Testnet. 
...
may result in unexpected bahavior like returning different Absolute address than the one for which 
the query was sent.